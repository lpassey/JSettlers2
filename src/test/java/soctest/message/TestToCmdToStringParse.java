...
public class TestToCmdToStringParse
{
    ...
    public void testRoundTripParsing()
    {
        ...
        for (Object[] compareCase : TOCMD_TOSTRING_COMPARES)
        {
            ...
            if (expectedToCmd != null)
            {
                ...
                {
                    res.append( " toCmd: expected \"" ).append( expectedToCmd ).append( "\", got \"" ).append( s ).append( "\"" );
                }
                else
                {
                    ...
                    {
                        res.append( " toMsg(cmd): got wrong class " ).append( rev.getClass().getSimpleName() );
                    }
                    else
                    {
            ...
            if (expectedToString != null)
            {
                ...
                {
                    res.append( " toString: expected \"" ).append( expectedToString ).append( "\", got \"" ).append( s ).append( "\"" );
                }
                else if (!skipParseToString)
                {
                    ...
                    try
                    {
                        ...
                        {
                            res.append( " parseMsgStr(s): got wrong class " ).append( rev.getClass().getSimpleName() );
                        }
                        else
                        {
                            compareMsgObjFields( msgClass, msg, rev, res, ignoreObjFields );
                    ...
                    {
                        res.append( " parseMsgStr(s) rejected: " ).append( e.getMessage() );
                    }
                    catch( ParseException e )
                    {
                        res.append( " parseMsgStr(s) failed: " ).append( e );
                    }
                }
            }
        ...
        for (Field f : msgClass.getDeclaredFields())
        {
            ...
            try
            {
                ...
                if (valueExpected == null)
                {
                    if (valueActual != null)
                    {
                        res.append( " field " ).append( fName ).append( ": expected null, got " ).append( str( valueActual ) );
                    }
                }
                else
                {
                    if ((valueExpected instanceof int[]) && (valueActual instanceof int[]))
                    {
                        if (!Arrays.equals( (int[]) valueExpected, (int[]) valueActual ))
                            res.append( " field " ).append( fName ).append( ": expected " ).append( str( valueExpected ) ).append( ", got " ).append( str( valueActual ) );
                    }
            ...
            {
                res.append( " error reading field " ).append( fName ).append( ": " ).append( e );
            }
        }
    }
     ...
     */
    private static String str(final Object val)
    {
        final String ret;
    ...
    private static final Object[][] TOCMD_TOSTRING_COMPARES =
        {
            ...
            // TODO SOCBoardLayout2
            {
                ...
                OPT_IGNORE_OBJ_FIELDS, new HashSet<>( Collections.singletonList( "opts" ))
            },
            {
            ...
            {new SOCChannelMembers( "cha", Collections.singletonList( "m" )), "1002|cha,m", "SOCChannelMembers:channel=cha|members=[m]"},  // shortest list
            ...
            {
                new SOCDebugFreePlace( "ga", 3, true ),
                "1087|ga,3,0,1",
                "SOCDebugFreePlace:game=ga|playerNumber=3|pieceType=ROAD|coord=0x1"
            },
            {
                new SOCDebugFreePlace( "ga", 3, SOCPlayingPiece.SETTLEMENT, 0x405 ),
                "1087|ga,3,1,1029",
                "SOCDebugFreePlace:game=ga|playerNumber=3|pieceType=SETTLEMENT|coord=0x405"
            },
            ...
            {new SOCDiceResult( "ga", 9 ), "1028|ga,9", "SOCDiceResult:game=ga|param=9"},
            {
                ...
                new HashSet<>( Arrays.asList( "playerNum", "playerRsrc", "playerResTotal" ))
            },
            {
                new SOCDiscard( "ga", new SOCResourceSet( 2, 1, 3, 1, 2, 0 )),
                "1033|ga,2,1,3,1,2,0",
            ...
            {new SOCGameElements( "ga", GEType.CURRENT_PLAYER, 1 ), "1096|ga|4|1", "SOCGameElements:game=ga|CURRENT_PLAYER=1"},
            {
                new SOCGameElements("ga", new GEType[]
                    {
                        GEType.DEV_CARD_COUNT,
                        GEType.ROUND_COUNT,
                        GEType.FIRST_PLAYER,
                        GEType.LONGEST_ROAD_PLAYER,
                        GEType.LARGEST_ARMY_PLAYER
                    },
                    new int[]{ 25, 2, 1, -1, -1 }),
            ...
            {new SOCGameMembers( "ga", Collections.singletonList( "p" )), "1017|ga,p", "SOCGameMembers:game=ga|members=[p]"},  // shortest list
            ...
            {
                new SOCLastSettlement( "ga", 2, 0x405 ),
                "1060|ga,2,1029",
                "SOCLastSettlement:game=ga|playerNumber=2|coord=405"},
            ...
            },
            {
                ...
                OPT_SKIP_PARSE,
                OPT_IGNORE_OBJ_FIELDS, new HashSet<>( Collections.singletonList( "optsStr" ))
                // TODO +stripAttribNames
            },
            ...
            {
                new SOCPlayDevCardRequest( "ga", SOCDevCardConstants.KNIGHT ),
                "1049|ga,9",
                "SOCPlayDevCardRequest:game=ga|devCard=9"
            },
            {
                new SOCPlayerElement( "ga", 1, SOCPlayerElement.SET, 105, 1 ),
                "1024|ga,1,100,105,1",
                "SOCPlayerElement:game=ga|playerNum=1|actionType=SET|elementType=STARTING_LANDAREAS|amount=1"
            },
            {
                new SOCPlayerElement( "ga", 2, SOCPlayerElement.LOSE, 4, 1, true ),
                "1024|ga,2,102,4,1,Y",
            ...
            {
                new SOCPotentialSettlements( "ga", 2, new ArrayList<>()),
                "1057|ga,2",
                "SOCPotentialSettlements:game=ga|playerNum=2|list=(empty)"
            },
            {
                new SOCPotentialSettlements( "ga", 3, new ArrayList<>( Arrays.asList( 0xc04, 0xe05, 0x60a ))),
                "1057|ga,3,3076,3589,1546",
                "SOCPotentialSettlements:game=ga|playerNum=3|list=c04 e05 60a "
            },
            // same as above, but no trailing space in toString's list:
            {
                new SOCPotentialSettlements( "ga", 3, new ArrayList<>( Arrays.asList( 0xc04, 0xe05, 0x60a ))),
                "1057|ga,3,3076,3589,1546",
                "SOCPotentialSettlements:game=ga|playerNum=3|list=c04 e05 60a", OPT_PARSE_ONLY
            },
            // v2.x forms: For easier comparison, these tests all have 1 node in each land area set instead of many
            {
                ...
                new SOCPotentialSettlements( "ga", 0, new ArrayList<>( Arrays.asList( 0xa04, 0xa08 )), 0,
                    new HashSet[] { null, 
                        new HashSet( Collections.singletonList( 0xc02 )),
                        new HashSet( Collections.singletonList( 0x408 )),
                        new HashSet( Collections.singletonList( 0xa0f )),
                        new HashSet( Collections.singletonList( 0x100c )) },
                        null ),
            ...
            },
            {
                ...
                new SOCPotentialSettlements
                    ( "ga", -1, null, 1,
                        new HashSet[]{null, new HashSet( Collections.singletonList( 0x802 )), new HashSet( Collections.singletonList( 0xc02 ))},
                        null ),
            ...
            },
            {
                ...
                new SOCPotentialSettlements
                    ( "ga", 3, null, 1,
                        new HashSet[]{null, new HashSet( Collections.singletonList( 0xa0f )), new HashSet( Collections.singletonList( 0x60a ))},
            ...
            },
            {
                ...
                new SOCPotentialSettlements
                    ( "ga", 3,
                        new ArrayList<>( Arrays.asList( 0xa04, 0xa08 )),
                        new int[][]{{0xc07, -0xc0b, 0xd07, -0xd0b, 0xe04, -0xe0a, 0xa03}} ),
            ...
            {
                new SOCPutPiece( "ga", 3, 0, 1034 ),
                "1009|ga,3,0,1034",
                "SOCPutPiece:game=ga|playerNumber=3|pieceType=ROAD|coord=0x40a"
            },
            {
                new SOCRejectConnection( "reason msg" ),
                "1059|reason msg",
                "SOCRejectConnection:reason msg"},
            {
                new SOCRejectOffer( "ga", 2 ),
                "1037|ga,2",
                "SOCRejectOffer:game=ga|playerNumber=2"
            },
            {
                new SOCRemovePiece( "ga", 2, SOCPlayingPiece.SHIP, 0xe04 ),
                "1094|ga,2,3,3588",
            ...
            {new SOCScenarioInfo( "KEY3", true ), "1101|KEY3|0|-2", "SOCScenarioInfo:key=KEY3|minVers=0|lastModVers=MARKER_KEY_UNKNOWN", OPT_SKIP_PARSE},
            {
                ...
                OPT_IGNORE_OBJ_FIELDS,
                new HashSet<>( Arrays.asList( "scKey", "noMoreScens" ))
            },
            {
            ...
            {
                new SOCSetSeatLock( "ga", 2, SeatLockState.LOCKED ),
                "1068|ga,2,LOCKED",
                "SOCSetSeatLock:game=ga|playerNumber=2|state=LOCKED"
            },
            {
                new SOCSetSeatLock( "ga", new SeatLockState[]
                    {
    ...
    public void testStripAttribsToList()
    {
        ...
        expected.add("xyz");
        assertEquals( expected, SOCMessage.stripAttribsToList( "xyz" ));
        assertEquals( expected, SOCMessage.stripAttribsToList( "param=xyz" ));

        // multi-element
        expected.add("abc");
        assertEquals( expected, SOCMessage.stripAttribsToList( "xyz|abc" ));
        assertEquals( expected, SOCMessage.stripAttribsToList( "xyz|param=abc" ));
        assertEquals( expected, SOCMessage.stripAttribsToList( "param=xyz|param=abc" ));
