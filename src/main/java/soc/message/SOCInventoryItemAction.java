...
public class SOCInventoryItemAction extends SOCMessageForGame
{
    ...
    public enum IIAction
    {
        ...
        public static IIAction valueOf( final int ti )
        {
            for (IIAction action : values())
            {
                if (action.value == ti)
                    return action;
            }
...
//    public static final int PLACING_EXTRA = 7;
     ...
     */
    public SOCInventoryItemAction(final String ga, final int pn, final IIAction ac, final int it)
    {
        this(ga, pn, ac, it, 0);
    }
    ...
    public SOCInventoryItemAction
        (final String ga, final int pn, final IIAction ac, final int it,
         final boolean kept, final boolean vp, final boolean canCancel)
    {
        super( INVENTORYITEMACTION, ga );
        playerNumber = pn;
//        action = IIAction.valueOf( ac );
        action = ac;
        itemType = it;
     ...
     */
    public SOCInventoryItemAction
        (final String ga, final int pn, final IIAction ac, final int it, final int rc)
    {
        super( INVENTORYITEMACTION, ga );
        playerNumber = pn;
//        action = IIAction.valueOf( ac );
    ...
    public static SOCInventoryItemAction parseDataStr(String s)
    {
        ...
        try
        {
            ...
            if (st.hasMoreTokens())
            {
                ...
                if (   (IIAction.valueOf( ac ) != IIAction.PLAY)
                    && (IIAction.valueOf( ac ) != IIAction.CANNOT_PLAY))
                {
                    actionHasFlags = true;
        ...
        if (actionHasFlags)
            return new SOCInventoryItemAction(ga, pn, IIAction.valueOf( ac ), it, kept, vp, canCancel);
        else
            return new SOCInventoryItemAction(ga, pn, IIAction.valueOf( ac ), it, rc);
    }
    ...
    {
        String s = "SOCInventoryItemAction:game=" + getGame() + "|playerNum=" + playerNumber
            + "|action=" + action.name() + "|itemType=" + itemType;

        if ((action != IIAction.PLAY) && (action != IIAction.CANNOT_PLAY))
