...
public final class MessageHandler implements SOCMessageDispatcher
{
        ...
        try
        {
            ...
            case SOCMessage.LASTSETTLEMENT:
                SOCDisplaylessPlayerClient.handleLASTSETTLEMENT
                    ( (SOCLastSettlement) mes, client.games.get( ((SOCLastSettlement) mes).getGameName() ) );
                break;
            ...
            case SOCMessage.SCENARIOINFO:
                handleSCENARIOINFO( (SOCScenarioInfo) mes, connection );
                break;
            ...
            case SOCMessage.REPORTROBBERY:
                handleREPORTROBBERY
                    ((SOCReportRobbery) mes, client.games.get(((SOCMessageForGame) mes).getGameName()));
                break;
            ...
            case SOCMessage.PICKRESOURCES:
                handlePICKRESOURCES
                    ((SOCPickResources) mes, client.games.get(((SOCMessageForGame) mes).getGameName()));
                break;
    ...
    protected void handleSTATUSMESSAGE( SOCStatusMessage mes )
    {
        ...
        if (client.isNGOFWaitingForAuthStatus)
        {
    ...
    protected void handleNEWGAME( SOCNewGame mes )
    {
        ...
        client.addToGameList( mes.getGameName(), null, true );
    }

    /**
     * handle the "delete game" message. Removes the game from the list of joinable games on the server.
     *
     * @param mes  the message
     */
    protected void handleDELETEGAME(SOCDeleteGame mes )
    {
        ...
        EventQueue.invokeLater( new Runnable()
        {
            public void run()
            {
                ...
                if (!mdisp.deleteFromGameList( gaName, false ))
                    mdisp.deleteFromGameList( gaName, true );
    ...
    protected void handleSITDOWN( final SOCSitDown mes, Connection connection )
    {
         ...
         */
        final SOCGame ga = client.games.get( mes.getGameName() );
        if (ga == null)
            return;
        ...
        final boolean playerIsClient = client.getNickname().equals( plName );

        if (   playerIsClient
            && ((client.getConnection().getRemoteVersion() >= SOCDevCardAction.VERSION_FOR_SITDOWN_CLEARS_INVENTORY)))
        {
    ...
    {
        SOCGame ga = client.games.get( mes.getGameName() );
        if (ga == null)
            return;
    ...
    protected void handlePLAYERELEMENTS( SOCPlayerElements mes )
    {
        ...
        final PlayerClientListener pcl = client.getClientListener( mes.getGameName() );
        final int pn = mes.getPlayerNumber();
    ...
    protected void handlePLAYERELEMENT( SOCPlayerElement mes )
    {
        ...
        handlePLAYERELEMENT( client.getClientListener( mes.getGameName() ), ga, null, pn, action,
        ...
        final int pn, final int action, final PEType etype, final int amount, final boolean isNews )
    {
        ...
        if (null == pl)
            return;     // probably unnecessary, but prevents IDE warnings
    ...
    protected void handleRESOURCECOUNT( SOCResourceCount mes )
    {
        ...
        handlePLAYERELEMENT
            ( client.getClientListener( mes.getGameName() ), ga, null, mes.getPlayerNumber(),
    ...
    protected void handleMOVEROBBER( SOCMoveRobber mes )
    {
        ...
        /**
         * Note: Don't call SOCGame.moveRobber() because that will call the
    ...
    {
        SOCGame game = client.games.get( mes.getGameName() );
    ...
    {
        SOCGame theGame = client.games.get( mes.getGameName() );
        if (theGame == null)
            return;
     ...
     * by calling {@link ServerGametypeInfo#receiveInfo(SOCGameOptionInfo, SOCStringManager)}.
