    ...
    implements ActionListener, MouseListener
{
    ...
    public SOCHandPanel( final SOCPlayerInterface pi, final SOCPlayer pl, final boolean isInteractive )
    {
        ...
        createColorSqRetSq = null;  // done, clear refs
...
*/
        ...
        if (!playerTradingDisabled) // playerSend, playerSendMap, playerSendForPrevTrade already null
        {
        ...
        else
        {
            ...
            offerPanel = new TradePanel
                    ...
                    false, true, this, tradeInteriorColor, new TradePanel.TPListener()
                {
                    ...
                    {
                        clickOfferAcceptButton();
                    }

                    public void button2Clicked()
                    {
                        clickOfferRejectButton();
                    }

                    public void button3Clicked()
                    {
                        clickOfferCounterButton();
                    }
                }, displayScale );
            ...
            counterOfferPanel = new TradePanel
                    ...
                    true, false, this, tradeInteriorColor, new TradePanel.TPListener()
                {
                    public void button1Clicked()
                    {
                        clickCounterOfferSendButton();
                    }

                    public void button2Clicked()
                    {
                        clickCounterOfferClearButton();
                    }

                    public void button3Clicked()
                    {
                        clickCounterOfferCancelButton();
                    }
                }, displayScale );
    ...
    public void actionPerformed( ActionEvent e )
    {
        try
        {
            ...
            if (target != ROBOT)    //  if true, cf.cc.addRobot(cf.cname, playerNum);
            {
                if (target == LOCKSEAT)
                {
            ...
            if (playerIsClient)
            {
                ...
                if (offerBut != null)
                {
                    if (game.maxPlayers == 4)
                        offerBut.setBounds( inset, tbY, tbW, lineH );
                    else  // 6-player: leave room for 5 checkboxes
                        offerBut.setBounds( inset, tbY, (2 * tbW) + space - (5 * (1 + sqSize)), lineH );
                }
                clearOfferBut.setBounds( inset, tbY + lineH + space, tbW, lineH );
                bankBut.setBounds( inset + tbW + space, tbY + lineH + space, tbW, lineH );
                bankUndoBut.setBounds( inset + tbW + space, tbY + 2 * (lineH + space), tbW, lineH );
                ...
                if (!playerTradingDisabled)
                {
                    ...
                    if (game.maxPlayers == 4)
                    {
                        playerSend[0].setLocation( inset + tbW + space, tbY );
                        playerSend[1].setLocation( inset + tbW + space + ((tbW - sqSize) / 2), tbY );
                        playerSend[2].setLocation( (inset + tbW + space + tbW) - sqSize, tbY );
                    }
                    else
                    {
                        // 6-player: 5 checkboxes
                        int px = inset + (2 * (space + tbW)) - sqSize - 1;
