 ...
 * {@link soc.baseclient.InProcessConnection}'s reader thread calls
 * {@link #handle(SOCMessage, boolean)} to dispatch messages to these
 * handler methods.
 *<P>
 *
 * Contains only methods which override some methods in the
 * {@link PlayerMessageHandler} super class.
 *
 * @author lee passey
 * @since 2.8.00
 */
// @SuppressWarnings("ConstantConditions")
public class InProcMessageHandler extends PlayerMessageHandler
{
    /**
     * Get this PlayerMessageHandler's client.
     * @since 2.5.00
     */
    protected SOCFullClient getClient()
    {
     ...
     */
    public InProcMessageHandler( SOCPlayerClient client )
    {
        super( client );
    }
...
//        }
        ...
        final boolean withTokenI18n =
               ( clientLocale != null)
//            && (/* isPractice || */(playerClient.sVersion >= SOCStringManager.VERSION_FOR_I18N))
            && ! ("en".equals( clientLocale.getLanguage())
            && "US".equals( clientLocale.getCountry()));
        
//        SOCGameOptionSet opts3p = null;
...
//            else
//            {
                final SOCGameOptionGetInfos ogiMsg = new SOCGameOptionGetInfos(null, withTokenI18n, withTokenI18n && sameVersion);
//                    // sends "-" and/or "?I18N"
//            }

//            if (! isPractice)     // always true for TCPMessageHandler
//            playerClient.getMainDisplay().optionsRequested();
            getClient().sendPractice( ogiMsg.toCmd() );
//        }
//        else
//        if (false)
//        {
...
//        else
        {          
            ...
            // For practice games, knownOpts may already be initialized, so check vs null. TODO: Why?
            ServerGametypeInfo opts = ((SOCFullClient)client).practiceServGameOpts ;
            if (opts.knownOpts == null)
                opts.knownOpts = SOCGameOptionSet.getAllKnownOptions();
            opts.noMoreOptions( true  );  // if practice, all the defaults are known
...
//        if ( /* isPractice || */ (getClient().sVersion >= 2000))
        {
...
//        if ((false) && getClient().isNGOFWaitingForAuthStatus)
//        {
...
//                    public void run()
//                    {
//                        getClient().getMainDisplay().gameWithOptionsBeginSetup(false, true);
//                    }
//                });
//            }
//        }
        ...
        case SOCStatusMessage.SV_NEWGAME_OPTION_VALUE_TOONEW:
        {
            ...
            try
            {
                ...
                final SOCGameOptionSet knowns = getClient().practiceServGameOpts.knownOpts;
//                    (isPractice) ? getClient().practiceServGameOpts.knownOpts : getClient().tcpServGameOpts.knownOpts;
        ...
        case SOCStatusMessage.SV_SERVER_SHUTDOWN:
        {
            ...
            getClient().inProcessConnection.setLastException( new RuntimeException(statusText) );
    ...
    protected void handleSERVERPING( SOCServerPing mes )
    {
        ...
        if (timeval != -1)
        {
            getClient().sendPractice( mes.toCmd() );
        }
        else
        {
            // if timeval is < 0, we are being kicked by player with same name
            // should never happen in messages from the practice server!
            getClient().inProcessConnection.setLastException(
...
//    }
    ...
    protected void handleCHANNELS( final SOCChannels mes )
    {
        EventQueue.invokeLater( () -> {
            ...
            mdisp.channelList(mes.getChannels(), true );
    ...
    protected void handleGAMES(final SOCGames mes )
    {
        ...
        EventQueue.invokeLater( () -> {
            for (String gn : mes.getGames())
                getClient().addToGameList(gn, null, false);
...
//        getClient().setAuthenticated( true );
        ...
        // get known options for practice games
        final SOCGameOptionSet knownOpts = getClient().practiceServGameOpts.knownOpts;
 ...
 //              This call will get the game options directly from the in process server.
        SOCGame practiceGame = getClient().getPracticeGame( gaName );
        ...
        ga.isPractice = true;
        ga.serverVersion = Version.versionNumber(); // Could probably get it from the client just as easily: getClient().sVersion;
...
//    }
    ...
    protected void handleNEWGAME(SOCNewGame mes )
    {
        ...
        getClient().addToGameList( mes.getGameName(), null, false );
    }
    ...
    protected void handleDELETEGAME(SOCDeleteGame mes )
    {
        ...
        EventQueue.invokeLater( () -> {
            ...
            if (! mdisp.deleteFromGameList(gaName, true, false))
            {
                mdisp.deleteFromGameList( gaName, true, true );
            }
...
//    }
    ...
    protected SOCGame handleSITDOWN(final SOCSitDown mes)
    {
        ...
        if (playerIsClient && ! ga.isBoardReset())
        {
            ...
            getClient().sendPractice(new SOCChangeFace(ga.getName(), 0, id).toCmd() );
        }
        return ga;
    }
...
//    }
     ...
     * {@link super#handleGAMEELEMENT(SOCGame, GEType, int)}.
     * <p/>
...
//    private void handleGAMEELEMENTMH( SOCGame ga, GEType etype, int value )
//    {
...
//                handleGAMEELEMENT(ga, etype, value);
...
//        {
//        case ROADS:
//            super.handlePLAYERELEMENT_numPieces( pl, action, SOCPlayingPiece.ROAD, amount);
//            utype = PlayerClientListener.UpdateType.Road;
//            break;
//
//        case SETTLEMENTS:
//            super.handlePLAYERELEMENT_numPieces(pl, action, SOCPlayingPiece.SETTLEMENT, amount);
//            utype = PlayerClientListener.UpdateType.Settlement;
//            break;
//
//        case CITIES:
//            super.handlePLAYERELEMENT_numPieces( pl, action, SOCPlayingPiece.CITY, amount);
//            utype = PlayerClientListener.UpdateType.City;
//            break;
//
//        case SHIPS:
//            super.handlePLAYERELEMENT_numPieces( pl, action, SOCPlayingPiece.SHIP, amount);
...
//                super.handlePLAYERELEMENT_numKnights( ga, pl, action, amount);
...
//
//        case CLAY:
//            super.handlePLAYERELEMENT_numRsrc( pl, action, SOCResourceConstants.CLAY, amount);
//            utype = PlayerClientListener.UpdateType.Clay;
//            break;
//
//        case ORE:
//            super.handlePLAYERELEMENT_numRsrc( pl, action, SOCResourceConstants.ORE, amount);
//            utype = PlayerClientListener.UpdateType.Ore;
//            break;
//
//        case SHEEP:
//            super.handlePLAYERELEMENT_numRsrc( pl, action, SOCResourceConstants.SHEEP, amount);
//            utype = PlayerClientListener.UpdateType.Sheep;
//            break;
//
//        case WHEAT:
//            super.handlePLAYERELEMENT_numRsrc( pl, action, SOCResourceConstants.WHEAT, amount);
//            utype = PlayerClientListener.UpdateType.Wheat;
//            break;
//
//        case WOOD:
//            super.handlePLAYERELEMENT_numRsrc( pl, action, SOCResourceConstants.WOOD, amount);
...
//             */
//            super.handlePLAYERELEMENT_numRsrc( pl, action, SOCResourceConstants.UNKNOWN, amount);
//            utype = PlayerClientListener.UpdateType.Unknown;
//            break;
//
//        case ASK_SPECIAL_BUILD:
//            super.handlePLAYERELEMENT_simple( ga, pl, pn, action, etype, amount, null);
...
//            return;
//
//        final SOCPlayer pl = super.handleDISCARD(mes, ga);
//        if (pl == null)
//            return;
//
...
//            return;
//
//        super.handleACCEPTOFFER(mes, ga);
//
//        PlayerClientListener pcl = getClient().getClientListener(gaName);
//        if (pcl == null)
//            return;
...
//    }
    ...
    protected void handleBANKTRADE(final SOCBankTrade mes )
    {
        ...
        // static method, so PlayerMessageHandler.handleBANKTRADE will not be called
...
//            return;
//
//        MessageHandler.handlePLAYERELEMENT_simple
...
//    {
//        final String gaName = mes.getGameName();
//        MessageHandler.handlePOTENTIALSETTLEMENTS(mes, getClient().getGame(gaName));
//
//        PlayerClientListener pcl = getClient().getClientListener(gaName);
//        if (pcl != null)
...
//    {
//        // TODO: this needs a lot of work, because a client may have two connections. I don't think
//        // a practice server will ever reject a connection!
...
//    }
    ...
    {
        ServerGametypeInfo servOpts = getClient().practiceServGameOpts;
        ...
        if (unknowns != null)
        {
             getClient().sendPractice( new SOCGameOptionGetInfos(unknowns, getClient().wantsI18nStrings(true ), false).toCmd());
        }
        else
        {
            servOpts.newGameWaitingForOpts = false;
            getClient().getMainDisplay().optionsReceived( servOpts, true );
        }
    }
     ...
     */
    @Override
    protected void handleGAMEOPTIONINFO( SOCGameOptionInfo mes )
    {
        ServerGametypeInfo opts = getClient().practiceServGameOpts;
        boolean hasAllNow;
        synchronized(opts)
        {
        ...
        getClient().getMainDisplay().optionsReceived(opts, true, isDash, hasAllNow);
    }
    ...
    protected void handleNEWGAMEWITHOPTIONS(final SOCNewGameWithOptions mes )
    {
        ...
        boolean canJoin = true;     // Practice games are always joinable
        ...
        getClient().getMainDisplay().addToGameList( ! canJoin, gname, opts, false );
    }
    ...
    protected void handleGAMESWITHOPTIONS(SOCGamesWithOptions mes )
    {
        ...
        final SOCGameList msgGames = mes.getGameList( getClient().practiceServGameOpts.knownOpts);
        if (msgGames == null)
            return;
    ...
    protected void handleLOCALIZEDSTRINGS(final SOCLocalizedStrings mes /*, final boolean isPractice */)
    {
        ...
        {
//           getClient().localizeGameScenarios( strs, true, mes.isFlagSet(SOCLocalizedStrings.FLAG_SENT_ALL), isPractice);
        }
        else
        {
...
//            return;  // Not one of our games
//
//        MessageHandler.handlePLAYERSTATS(mes, pcl.getGame(), clientPN);  // update player data
//
//        final int stype = mes.getStatType();
//        switch (stype)
//        {
...
//            super.handleSIMPLEACTION(mes, getClient().getGame(gaName));
...
//
//        // Call static method
//        handleDICERESULTRESOURCES(mes, ga, null, true);
...
//                handleINVENTORYITEMACTION( mes, getClient().getGame(mes.getGameName()));
//
//        PlayerClientListener pcl = getClient().getClientListener( mes.getGameName() );
//        if (pcl == null)
//            return;
...
//    {
//        // update game data:
//        super.handleSETSPECIALITEM( (SOCSetSpecialItem) mes, getClient().getGame( mes.getGameName() ));
//
//        final PlayerClientListener pcl = getClient().getClientListener(mes.getGameName());
//        if (pcl == null)
//            return;
...
//    {
//        SOCGame ga = getClient().getGame( mes.getGameName() );
//        super.handleROBBERYRESULT(mes, ga);
//
//        PlayerClientListener pcl = getClient().getClientListener(mes.getGameName());
//        if (pcl != null)
...
//        if (! super.handlePICKRESOURCES(mes, ga))
//            return;
//
//        PlayerClientListener pcl = getClient().getClientListener(ga.getName());
//        if (pcl != null)
